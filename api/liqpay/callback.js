...
const { buildPaidOrderMessage } = require('../_lib/order-utils');
const { buildSignature, fetchLiqPayPaymentStatus } = require('../_lib/liqpay');
...
module.exports = async (req, res) => {
    ...
    if (statusGroup === 'paid') {
        try {
            ...
            const message = buildPaidOrderMessage({
                ...payload,
                ...(statusPayload && typeof statusPayload === 'object' ? statusPayload : {})
            });
            await sendTelegramMessage(message);
        } catch (error) {
            console.error('Failed to notify Telegram about paid LiqPay order.', error);
        }
