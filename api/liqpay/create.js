...
const { saveOrder } = require('../_lib/order-store');
...
module.exports = async (req, res) => {
    ...
    const resultUrl = String(process.env.LIQPAY_RESULT_URL || '').trim();
    ...
    if (!resultUrl || !serverUrl) {
        res.status(500).json({ error: 'LIQPAY_RESULT_URL and LIQPAY_SERVER_URL must be configured on server.' });
        return;
    }
    ...
    try {
        ...
        console.error('Failed to save LiqPay order before checkout init.', storeError);
