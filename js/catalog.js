const Catalog = {
    ...
    openOrderModal() {
        ...
        if (fullNameInput && !fullNameInput.value) fullNameInput.value = orderDefaults.name || '';
        if (phoneInput && !phoneInput.value) phoneInput.value = orderDefaults.phone || '';
            ...
            form.addEventListener('submit', async (event) => {
                ...
                try {
                    for (const cartEntry of cartItems) {
                        const deliveryComment = comment
                            ? `Доставка: ${shipping}\nОпис: ${comment}`
                            : `Доставка: ${shipping}`;

                        const orderResult = await window.SiteAuth?.saveCatalogOrder?.({
                            item: cartEntry.item,
                            quantity: this.normalizeQuantity(cartEntry.quantity),
                            contact_name: name,
                            contact_phone: phone,
