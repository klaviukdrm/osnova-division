    ...
    async loadProductsFromApi() {
        try {
            ...
            const mappedProducts = sourceProducts
                .map((product) => this.mapApiProductToCatalog(product))
            ...
            const orderedCategories = [
                ...
                ...uniqueCategories.filter((category) => !categoryOrder.includes(category))
            ];

            this.setCatalogData(orderedCategories.length ? orderedCategories : [this.BASE_APPAREL_LABEL], mappedProducts);
            return true;
        } catch (error) {
    ...
    setupOrderModalEvents() {
        ...
        if (form) {
            ...
            const processInvoiceOrder = async () => {
                ...
                if (!receiptImage) {
                    alert('Додайте скріншот або квитанцію про оплату перед оформленням замовлення.');
                    return;
                }
                ...
                try {
                    ...
                    alert(
                        `Замовлення оформлено (оплата за реквізитами).\nНомер: ${result?.orderId || 'невідомий'}\nПІБ: ${orderPayload.name}\nМісто: ${orderPayload.city}\nТелефон: ${orderPayload.phone}\nДоставка: ${orderPayload.shipping}\nПозицій: ${this.getCartItemsCount()}`
                    );
                    this.clearCart();
                    closeInvoiceModal();
                    this.closeOrderModal();
            ...
            const processWalletPayment = async () => {
                ...
                try {
                    ...
                    window.sessionStorage.setItem('upf_pending_liqpay_order', JSON.stringify({
                    ...
                    alert(error?.message || 'Не вдалося перейти до LiqPay. Спробуйте ще раз.');
                    setPaymentButtonsState(false);
                }
            };
            ...
            if (invoiceReceiptInput) {
                invoiceReceiptInput.addEventListener('change', async () => {
                    ...
                    if (!isImage) {
                        alert('Дозволено завантажувати тільки зображення (скріншот/квитанцію).');
                        resetInvoiceReceiptState();
                        return;
                    }
                    ...
                    if (selectedFile.size > maxBytes) {
                        alert('Файл занадто великий. Максимум 12 МБ.');
                        resetInvoiceReceiptState();
                        return;
                    }
                    ...
                    try {
                        ...
                        alert('Не вдалося обробити файл. Спробуйте інше зображення.');
                        resetInvoiceReceiptState();
                    }
                });
            }
            ...
            document.querySelectorAll('[data-copy-invoice]').forEach((button) => {
                ...
                button.addEventListener('click', async () => {
                    ...
                    if (ok) {
                        ...
                        alert('Не вдалося скопіювати. Скопіюйте реквізити вручну.');
                    }
                });
            });
