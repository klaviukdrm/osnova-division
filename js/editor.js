...
const Editor = {
    ...
    imageResizeState: { active: false, pointerId: null, axis: 'both', startScale: 1, startDistance: 1, centerX: 0, centerY: 0 },
    ...
    bindEvents() {
        ...
        if (this.elements.imageOrientationControls) {
            this.elements.imageOrientationControls.addEventListener('click', (event) => {
                const button = event.target.closest('[data-image-orientation]');
                if (!button) return;
                this.setImageOrientation(button.getAttribute('data-image-orientation'));
            });
        }
    ...
    applyImageTransformToImage(image, options = {}) {
        ...
        const state = this.getImageState(image);
        image.style.transform = `translate(calc(-50% + ${state.x}px), calc(-50% + ${state.y}px)) scale(${state.scale}) rotate(${state.rotate}deg)`;
    ...
    startImageResize(event, handle) {
        ...
        this.imageResizeState = {
            ...
            axis,
            startScale: this.getImageState(this.previewImageEl).scale,
            startDistance,
            centerX,
            centerY
        };
    ...
    addToCartDemo() {
        ...
        const itemTitle = `${product.name} • ${format.label}`;
        const cartItem = {
            title: itemTitle,
        ...
        try {
            ...
            const itemKey = `${cartItem.category || ''}::${cartItem.title || ''}`;
