...
const Editor = {
    ...
    applyInitialRouteState() {
        ...
        if (requestedVariant) {
            const targetProductId = requestedProduct && this.products.some((product) => product.id === requestedProduct)
                ? requestedProduct
                : this.state.productId;
            const targetProduct = this.products.find((product) => product.id === targetProductId);
            const hasVariant = Array.isArray(targetProduct?.variants)
                && targetProduct.variants.some((variant) => variant.id === requestedVariant);
            if (hasVariant) {
                this.state.variantByProduct[targetProductId] = requestedVariant;
            }
        }
    ...
    collectCurrentSourceImages() {
        ...
        layers.forEach((layer) => {
            ...
            const isDataFile = /^data:[^;,]+(?:;[^,]*)?;base64,/i.test(normalizedSource);
    ...
    filterUploadImageFiles(fileList) {
        return Array.from(fileList || []).filter((file) => this.isLikelyImageFile(file));
    },
    ...
    async getOptimizedUploadDataUrl(file) {
        ...
        const mimeType = String(file.type || '').trim().toLowerCase();
        if (!CANVAS_OPTIMIZABLE_MIME_TYPES.has(mimeType)) {
            return { dataUrl: await this.readFileAsDataUrl(file), optimized: false };
        }
        ...
        try {
            let image;
            try {
                image = await this.loadImageFromObjectUrl(objectUrl);
            } catch (_) {
                return { dataUrl: await this.readFileAsDataUrl(file), optimized: false };
            }
